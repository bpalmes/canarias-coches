...
export default async function AdminInventoryPage({
    ...
    if (!session || (!session.user.dealershipId && session.user.role !== 'SUPER_ADMIN')) {
        // Allow Super Admin to pass through even without dealershipId (viewing all? or impersonating)
    ...
    const dealership = dealershipId ? await prisma.dealership.findUnique({
        ...
        select: { financingEnabled: true, usePlatformFinancing: true }
    }) : null

    // Financing TAB is ONLY for Super Admins to configure/view bulk financing
    // OR maybe sophisticated dealers? 
    // User request: "el tab de configuracion de la calculadora es solo para supr admin"
    // If this refers to the Inventory > Financing tab which has bulk calculation, let's restrict it.

    // Check if we are physically the SUper Admin AND not impersonating
    // session.user.role is 'SUPER_ADMIN' even when impersonating.
    // dealershipId (from getActiveDealershipId) is NOT undefined if impersonating.
    // But session.user.dealershipId IS undefined for real super admin.

    // Real Super Admin (not impersonating): session.user.dealershipId is null, activeDealershipId is undefined
    // Impersonating: session.user.dealershipId is null, activeDealershipId is 123
    // Dealer: session.user.dealershipId is 123, activeDealershipId is 123

    const isRealSuperAdmin = session.user.role === 'SUPER_ADMIN' && !dealershipId;
    const showFinancingTab = isRealSuperAdmin && dealership?.financingEnabled;
    const view = typeof resolvedSearchParams.view === 'string' ? resolvedSearchParams.view : 'inventory'
    ...
    if (view === 'financing' && showFinancingTab) {
        // Financing View: fetch all (or large page) to show table
    ...
    return (
        <div className="px-4 sm:px-6 lg:px-8 py-8">
                ...
                <div className="mt-4 sm:mt-0 sm:ml-16 sm:flex-none flex items-center gap-3">
                    {showFinancingTab && (
                        <div className="flex bg-gray-100 p-1 rounded-lg mr-4">
                            <Link
                                href="/admin/inventory?view=inventory"
                                className={`px-4 py-2 text-sm font-medium rounded-md ${view !== 'financing' ? 'bg-white shadow text-gray-900' : 'text-gray-500 hover:text-gray-900'}`}
                            >
                                Inventario
                            </Link>
                            <Link
                                href="/admin/inventory?view=financing"
                                className={`px-4 py-2 text-sm font-medium rounded-md ${view === 'financing' ? 'bg-white shadow text-gray-900' : 'text-gray-500 hover:text-gray-900'}`}
                            >
                                Financiación
                            </Link>
            ...
            {view === 'financing' && showFinancingTab ? (
                // @ts-ignore
                <InventoryFinancingTable cars={cars} dealershipId={dealershipId!} />
            ) : (
                <>
                    {/* Filters */}
                            ...
                            <div className="inline-block min-w-full py-2 align-middle md:px-6 lg:px-8">
                                ...
                                <div className="mt-4 flex justify-between items-center text-sm text-gray-700">
                                    ...
                                    <div className="flex gap-2">
                                        {page > 1 && (
                                            <Link href={`/admin/inventory?page=${page - 1}${view ? `&view=${view}` : ''}`} className="px-3 py-1 border rounded hover:bg-gray-50">
                                                Anterior
                                            </Link>
                                        )}
                                        {page < meta.lastPage && (
